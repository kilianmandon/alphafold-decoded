# Tensor Introduction
## Intro
Hi everyone, and welcome to our introduction to tensors in PyTorch! I'm Kilian Mandon, and this video is the first in our series where we'll be [diving deep into the world of protein structure prediction by] implementing AlphaFold from scratch.

Our journey starts here, with the fundamental building blocks of deep learning: Tensors. In this video, we'll cover:
- What tensors are and why they're essential in models like AlphaFold
- How to create and manipulate tensors in PyTorch
- Advanced topics like axis-wise operations, broadcasting, and the powerful `torch.einsum` function

Everything we discuss today forms the foundation for the rest of the series. You'll apply these concepts directly in the Jupyter notebooks that accompany each video, where you'll build AlphaFold piece by piece. So, let's dive into tensors and take the first step towards unraveling the mysteries of protein structures!

Now, the best part about tensors is that, while the name might be new to you, you have probably worked with tensors before. Vectors for example are one dimensional tensors. Matrices are two-dimensional tensors. Operations like the dot-product between vectors, or matrix vector multiplication, are classic examples of operations on tensors.
...
So, to sum up: tensors are n-dimensional arrays that can represent everything in machine learning, from the images (or in AlphaFold's case sequence alignment data) that we feed into a model to the predictions it outputs. PyTorch's job is twofold: first, to provide efficient operations on these tensors, and second, to automatically compute the derivatives needed for learning. With these foundations in place, let's dive into how we create and work with tensors in PyTorch.
...
There are some other important tensor creation routines, that we'll get to know better in the tutorials. `torch.linspace` for example is a method that is used very often. It creates evenly spaced values between a specified start and end. `torch.linspace(0, 2, steps=5)` would create the tensor `[0, 0.5, 1, 1.5, 2]`. Another important method is `torch.ones`, which takes a desired shape, and creates a tensor of this shape where every value is 1.0. You can scale this tensor to any other value. This line creates a 4x3x4 tensor of 5s: `torch.ones((4,3,4)) * 5`
...
`[[0, 1], [2, 3], [4, 5]]`
This use of "rearrangement reshaping" is not the most common. More often, we will use reshape to flatten some dimensions, like reshaping (N, H, W) to (N, H*W), or for unflattening, which is just the reverse. A common use-case for reshaping also is adding 1-dimensions.

~ 2:30 Minutes

In addition to reshaping, *indexing* and *slicing* are fundamental tasks when working with tensors. Indexing is about accessing an element at a specific position in the tensor. If we have this matrix for example
`A = torch.tensor([[0,1,2],[3,4,5]])`
and we wanted to access the element 3, we would index to it using `A[1, 0]`, as it is in the second row (index 1) and first column (index 0). One thing to note is that when indexing elements in pytorch, you don't directly get the elements as numbers, but as one-element tensors. This is because PyTorch wants to keep track of values in its specific format, for example to allow for automatic differentiation. If you want to access the actual value as a number, you will need to write this as `A[2, 0].item()`.
...
For the translation, we want the first 3 rows, but only the fourth column. We can write this as `t = T[:3, 3]`. As the fourth row is the last row, we could also write this as `t = T[:3, -1]`, using negative indexing. With this syntax, t would be a one-dimensional tensor. But we might want to concatenate $R$ and $t$ again later into a 3x4 tensor. For this, R and t would need the same number of dimensions, R would be a 3x3 tensor and t a 3x1 tensor. To achieve this, we could also use slicing in the following way: `t = T[:3, 3:4]`, or `t=T[:3, 3:]`, as going to the end is the default. This pattern of using one-element slices, like 3:4, is common to conserve one-dimensions.

Now, we might not work with a single transform `T` of shape (4, 4), but with a batch of transforms of shape (N, 4, 4). Or we might even have one transform for each backbone in the batch, so we would have shape (N, N_res, 4, 4), where N_res is the number of residues. To account for this, PyTorch provides the ellipsis operator `...` which is really useful. It expands to as many `:` as are necessary to match the dimensions. The syntax `R = T[..., :3, :3]` correctly extracts the rotation matrices. For the batched case, `...` would expand to `:`. For the double-batched case, it would be `:, :` and for non-batch use, it's omitted.
...
Welcome to part 2 of our Introduction to Tensors! In the first part, we covered many of the basics regarding tensors, like the creation of tensors, indexing and reshaping. In this part, we want to highlight some more advanced concepts, in particular axis-wise operations, broadcasting and the einsum method.

## Axis-wise Operations
So far, we've covered the basics of tensor creation, indexing, and reshaping. These are the building blocks you'll use constantly. But as you work with more complex data, you'll need more advanced tools. 

One such tool is axis-wise operations. Let's say you have a 4x3 matrix and you want to sum up the elements. You have three  different options: Summing up all the rows (resulting in a 3-element tensor), summing up all the columns (resulting in a 4-element tensor), or summing up all elements (resulting in a 1-element tensor). You can specify the behaviour by setting the `dim` argument in the method `torch.sum`. If set to `dim=0`, summation will happen along the row dimension. If set to `dim=1` summation will happen along the column dimension. If set to `None`, all elements will be summed up. You can also set it to a tuple: Let's say you had a batch of matrices, of shape (N, 4, 3). You can calculate the sum of all elements for the matrices individually as `torch.sum(A, dim=(1, 2))` or `torch.sum(A, dim=(-1,-2))` using negative indexing, resulting in an N-element vector.
...
There are also axis-wise operations that aren't reducing. One that we will use in the tutorials is the softmax function. The sofmax function takes a vector $\bold{x}$ and creates a new vector according to
...
- If one value in the input is significantly larger than the others (say larger by ~6), the result will be close to a one-hot vector where only this entry is 1 and the others are 0, since the exponential function boosts large values overproportionally strong
...
* the dimensions of A and B are aligned with each other. Here, this would mean the following: $$\begin{matrix}A: & 4 & 3 \\ & | & | \\B: & 1 & 3 \\ \end{matrix}$$
* to be broadcastable, the individual dimensions must be equal, or set to one for one of the tensors
* along these one-dimensions, the tensor is expanded by duplicating it, until it matches the shape of the other tensor
...
With this last rule, we could have left out the reshaping when adding `B` as a row vector to `A`. The shapes (4, 3) and (3,) are broadcastable, as they are aligned from right to left, and ones are prepended to match the number of dimensions.
...
Another usecase for broadcasting are 'each-with-each' operations. Let's say we have a 3-element vector `v` and a 2 element vector `w`. If we want to compute the product of each element of `v` with each element of `w`, we would get six elements in total. We can organize them in a 3x2 matrix, where the entry i,j is made up of `v[i] * w[j]`. 
$$\begin{pmatrix}v_1w_1 & v_1w_2 \\ v_2w_1 & v_2w_2 \\ v_3w_1 & v_3w_2\end{pmatrix} = \begin{pmatrix}v_1 & v_1 \\ v_2 & v_2 \\ v_3 & v_3\end{pmatrix} * \begin{pmatrix}w_1 & w_2 \\ w_1 & w_2 \\ w_1 & w_2\end{pmatrix}$$

This is called the outer product of `v` and `w`. We can calculate this using broadcasting, by reshaping `v` to a 3x1 column vector and `w` to a 1x2 row vector, followed by a broadcasted multiplication of the two. 

## torch.einsum
$$c_{ikj} = a_{ik} \cdot b_{kj}$$
$$\sum_{k}c_{ikj} = \sum_k a_{ik} b_{kj}$$
...
In this example, we see all typical elements of a tensor-tensor operation: Some of the dimensions (i and j) are present in one of the inputs but not the other, and are worked on in an 'each-with-each' fashion. One dimension (k) is present in both inputs and is missing in the result: After calculating the pointwise product of the aligned vectors, the dimension is contracted by summation. One dimension (N) is present in both and is just used for a batched, 'for-each' calculation. 
...
* if a letter is present in one of the inputs, but not in the other and kept in the result (like i and j), the other tensor is unsqueezed and broadcasted to include the dimension
* if a letter is present in both inputs, but not in the result (like k), it is contracted 
* if a letter is present in both inputs and the result, it is treated as a batched, 'for-each' dimension
...
`torch.einsum` is incredibly flexible. You will use it all the time throughout the series, and you will already see a lot of examples in the tutorial notebook for this tensor introduction. The construction of the equation string can be a bit tricky at the start, but you will quickly get used to it. 
...
## Conclusion
And there you have it! We've covered the essentials of tensors in PyTorch, from basic creation and manipulation to advanced operations like broadcasting and `torch.einsum`. This knowledge isn't just theoretical—it's the bedrock upon which we'll build our understanding of AlphaFold.

Next up, we have two more foundational videos on machine learning basics and attention mechanisms. After that, we'll dive into AlphaFold-specific modules: feature extraction, the Evoformer, input embeddings, 3D geometry, the structure module, and finally, putting it all together.

But learning is an active process. That's why each video comes with a Jupyter notebook. In today's notebook, you'll work with all the tensor operations we've covered. As the series progresses, you'll use these skills to implement each part of AlphaFold, with guidance and automatic checks to keep you on track.

If you're ready to turn this introduction into action, head over to the accompanying notebook. And if you found this video helpful, give it a thumbs up and subscribe. That way, you won't miss any part of our AlphaFold journey.

Thank you for joining me today. I'm excited to guide you through implementing one of the most groundbreaking models in computational biology. See you in the next video, where we'll lay down more foundations with an intro to machine learning. Happy coding!